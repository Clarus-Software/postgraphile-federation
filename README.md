...
> The directives are applied to the **printed SDL only**, never the executable schema — so PostGraphile's resolver metadata is untouched (directives matter solely at compose time). We deliberately avoid `extendSchema`/print-reparse of the live schema, which would drop PostGraphile's resolve-data planner metadata.
...
const schema = await createFederatedPostGraphileSchema(pool, ["my_schema"], {
  ...
  // Passed straight through to createPostGraphileSchema:
  postgraphileOptions: {
    dynamicJson: true,
    ignoreRBAC: false,
  ...
  // federationImports: ["@key", "@shareable", "@inaccessible", "@tag"],
});
```
...
| `options.federationImports` | `string[]` | Directives imported in the `@link`. |
